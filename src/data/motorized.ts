...
- 🔧 간편 설치: 기존 레일에 쉽게 설치 가능
  ...
  },
  {
    ...
    materials: ['프리미엄 알루미늄', '독일산 모터', '리튬 배터리', '스마트 센서'],
  ...
  },
  {
    ...
    sizes: ['소형 (3-5개 창)', '중형 (6-10개 창)', '대형 (11개 이상)', '전체 빌딩'],
  ...
  },
  {
    ...
    materials: ['스마트 원단', 'AI 음성 모듈', '초정밀 모터', '노이즈 캔슬링 마이크'],
  ...
  },
  {
    ...
    materials: ['프리미엄 패브릭', '블루투스 모터', '스마트 센서', '클라우드 모듈'],
    sizes: ['150x220cm', '180x240cm', '220x260cm', '맞춤 제작'],
    colors: ['엘리트 화이트', '프리미엄 그레이', '럭셔리 베이지', '클래식 네이비'],
    price: {
      from: 245000,
      currency: 'KRW',
    },
  ...
  },
  {
    ...
    materials: ['친환경 원단', '태양광 패널', '리튬 폴리머 배터리', '저전력 모터'],
 ...
 * 슬러그로 모터라이즈드 제품 찾기
 */
export function getMotorizedProductBySlug(slug: string): MotorizedProduct | undefined {
 ...
 */
export function getMotorizedProductsByCategory(category: string): MotorizedProduct[] {
...
export function getMotorizedCategories() {
  return Array.from(new Set(motorizedProducts.map(product => product.category)));
}
