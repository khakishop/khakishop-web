...
export const referenceProducts: Product[] = [
  {
    ...
    title: '강남 고급 주택 시공사례',
    description: '강남 고급 주택에 설치된 프리미엄 커튼과 블라인드 시공사례입니다. 모던한 인테리어와 완벽하게 조화를 이루는 맞춤형 솔루션을 제공했습니다.',
    ...
    materials: ['벨기에 리넨', '알루미늄 프레임', '스마트 모터', '황동 액세서리'],
    bestseller: true,
    gallery: [
  ...
  },
  {
    ...
    description: '서울 시내 모던 오피스 빌딩의 대규모 블라인드 설치 프로젝트입니다. 효율적인 업무 환경을 위한 최적의 채광 솔루션을 구현했습니다.',
  ...
  },
  {
    ...
    description: '부산 해운대의 부티크 호텔 전 객실에 설치된 럭셔리 커튼 시공사례입니다. 바다 뷰와 조화로운 우아한 디자인으로 완성했습니다.',
  ...
  },
  {
    ...
    title: '일산 메디컬센터 차광시설',
    description: '일산 대형 메디컬센터의 차광 및 프라이버시 보호를 위한 전문 블라인드 시공사례입니다. 의료 환경에 최적화된 솔루션을 제공했습니다.',
  ...
  },
  {
    ...
    description: '대전 소재 대학교 중앙도서관의 열람실과 세미나실에 설치된 스마트 블라인드 시공사례입니다. 학습 환경 최적화에 중점을 두었습니다.',
  ...
  },
  {
    ...
    description: '송도국제도시 프리미엄 아파트 단지 전체 동에 설치된 대규모 커튼 프로젝트입니다. 통일성과 개성을 동시에 구현했습니다.',
...
export const getAllReferenceProducts = (): Product[] => {
  return referenceProducts.sort((a, b) => (a.displayOrder || 999) - (b.displayOrder || 999));
};

export const getReferenceProductBySlug = (slug: string): Product | undefined => {
  return referenceProducts.find(product => product.slug === slug);
};

export const getReferenceProductsBySubcategory = (subcategory: string): Product[] => {
  return referenceProducts.filter(product => product.subcategory === subcategory);
};
