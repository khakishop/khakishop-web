...
export const collectionProducts: Product[] = collections.map((collection, index) => ({
...
export const getCollectionSubcategories = (): string[] => {
  const subcategories = collectionProducts.map((product) => product.subcategory).filter(Boolean);
...
export const getCollectionProductBySlug = (slug: string): Product | undefined => {
  return collectionProducts.find((product) => product.slug === slug);
};

export const getCollectionProductsBySubcategory = (subcategory: string): Product[] => {
  return collectionProducts.filter((product) => product.subcategory === subcategory);
};
