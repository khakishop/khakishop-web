...
export default function Home() {
  ...
  // Transform values for the "aesthetic" text animation with organic feel
  const rawY = useTransform(scrollYProgress, [0.1, 0.4], [0, -150], {
    ease: [0.25, 0.46, 0.45, 0.94] // Custom easeOutBack-style curve
  });
  ...
  const randomOffset = useTransform(scrollYProgress, (latest) => {
    return Math.sin(latest * 10) * 3; // Subtle wobble
  });

  const finalY = useTransform([y, randomOffset], ([yVal, offset]) => yVal + offset);
  ...
  return (
    <div className="bg-white">
      ...
      <section ref={heroRef} className="relative h-screen -mt-16">
        ...
        {/* Animated "aesthetic" Headline with Organic Motion */}
        <motion.div
          style={{ 
            y: finalY, 
            opacity, 
